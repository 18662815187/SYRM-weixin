...
<body>
	...
	<div class="body-box" style="float: right">
		<form id="jvForm" action="saveOrUpdate.do?optionName1=${optionName1}&isshow1=${isshow1}&pageNo1=${pageNo1}" method="post"
			enctype="multipart/form-data">
			<input type="hidden" value="${data.id}" name="id" /> <input
				type="hidden" value="${optionName1}" /> <input type="hidden"
				value="${isshow1}" /> <input type="hidden" value="${pageNo1}" />
						...
						<td width="80%" class="pn-fcontent"><select
							name="optionLevel">
								...
								<c:forEach items="${datas}" var="d">
									<option value="${d.id}"
										<c:if test="${d.id == data.optionLevel}">selected="selected"</c:if>>${d.optionName}</option>
								</c:forEach>
						</select></td>
					</tr>
					<tr>
						...
						<td width="80%" class="pn-fcontent"><input type="radio"
							name="isshow" value="1"
							<c:if test="${data.isshow}">checked="checked"</c:if> />可用 <input
							type="radio" name="isshow" value="0"
							<c:if test="${!data.isshow}">checked="checked"</c:if> />不可用</td>
					</tr>
				</tbody>
				<tbody>
					<tr>
