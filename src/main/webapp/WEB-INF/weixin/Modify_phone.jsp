	...
	function sbClick() {
		$.ajax({
			...
			success : function(data) {
				//alert(data);
				if (data.status == 1) {
					alert("验证成功，请填写新的联系方式！");
					window.location.href = "/modifyPwd/toVerfiy?id=" + ${pharmacy.id};
				} else {
			...
			error : function() {
				// view("异常！");    
				alert("网络异常，请联系管理员2");
				window.location.href = "/modifyPwd/toModifyPhone?id=" + $
				{
					pharmacy.id
				}
				;
	...
	<form action="/modifyPwd/czPwd" method="post"
		...
		<div class="body">
			<table class="body_table">
				<tr>
					<td colspan="2"><span style="font-size: 30px;"> 修改联系电话
					</span></td>
				</tr>
				<tr>
					<td width="80px"><span>验证原手机号码：</span><input type="hidden"
					...
					<td><input id="tel" name="tel" type="text"
						placeholder="输入手机号码" size="22" value="${pharmacy.tel}"
						readonly="readonly" /><span style="color: red">*</span> <input
						type="button" id="aa" value="点击发送验证码" /></td>
				...
				<tr>
					<td colspan="2"
						...
						type="button" name="button" value="下一步" height="50px"
